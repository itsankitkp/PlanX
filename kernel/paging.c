...
void initialise_paging()
{
    ...
    memset((char *)frames, 0, INDEX_FROM_BIT(nframes));
...
void page_fault(registers_t regs)
{
    ...
    fb_write("Page fault! ( ", WHITE);
    if (present)
    {
        fb_write("present ", WHITE);
    }
    if (rw)
    {
        fb_write("read-only ", WHITE);
    }
    if (us)
    {
        fb_write("user-mode ", WHITE);
    }
    if (reserved)
    {
        fb_write("reserved ", WHITE);
    }
    fb_write(") at 0x", WHITE);
